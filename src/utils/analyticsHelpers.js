 ...
 */
export function computeSongScores(filteredVotes, { trackVotes = false, maxPts = null } = {}) {
    const songScores = {}
...
export function buildSortedRounds(filteredVotes) {
    ...
    const sortedRoundIds = Object.keys(roundDates).sort((a, b) =>
        new Date(roundDates[a]) - new Date(roundDates[b])
    )
 ...
 */
export function buildStandingsHistory({ sortedRoundIds, songScores, allPlayerIds }) {
